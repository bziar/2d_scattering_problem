...
%% Параметры системы
M = 8;                % Максимальное гармоническое число
M1 = 6;
N = 1;                 % Количество частиц
r = 1;              % Базовый радиус частиц
...
%% INITIALIZATION
SYSTEM.particles = cell(1, N);
SYSTEM.oldCoeffs = cell(1, N);
...
for run = 1:num_runs
    ...
    if run == 1
        ...
        initial_params = scale * randn(4 * N, 1);
    end
...
for k = 1:N
    parVal = params(4*(k-1)+1:4*k);
    newCoeffs = [zeros(1, 4), parVal(:).'];
    SYSTEM.particles{k}.PerturbFull(newCoeffs - SYSTEM.oldCoeffs{k}, 5);
    SYSTEM.particles{k}.ShapeUpdate(newCoeffs);
    SYSTEM.particles{k}.CheckCondition();
    SYSTEM.oldCoeffs{k} = SYSTEM.particles{k}.shapeCoeffs;
    scaMatrices{k} = SYSTEM.particles{k}.scaMatrix;
    SYSTEM.particles{k}.GradientCalc();
    for s = 1:4
        scaMatrices_der{4*(k-1) + s} = SYSTEM.particles{k}.gradients{s};
    end
...
if draw
    mTest.FarFieldPlot();
     SYSTEM.particles{k}.ShapePlot();
end

y = y + 1 / real(mTest.targetFunc);

for p = 1:4*N
    grad(p) = grad(p) - (2 * real(mTest.scaMatrix_der{p})) / (real(mTest.targetFunc))^2;
end
end
